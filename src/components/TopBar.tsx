...
const STARS_KEY = 'gh_stars_cache'
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        if (typeof v === 'number' && Date.now() - ts < STARS_TTL) {
          setStars(v)
          return
        }
      }
    ...
    fetch(`https://api.github.com/repos/${GITHUB_REPO}`)
      ...
      .then(d => {
        if (typeof d.stargazers_count === 'number') {
          ...
          try {
            sessionStorage.setItem(STARS_KEY, JSON.stringify({ v: d.stargazers_count, ts: Date.now() }))
          } catch {}
        }
      })
      .catch(() => {})
