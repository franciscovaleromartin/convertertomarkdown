...
const translations = {
  es: {
    ...
    faqA8: 'Depende del tipo de archivo. Los PDFs con texto digital (exportados desde Word, generados con software) se convierten directamente. Los PDFs escaneados —donde cada página es una imagen— no se procesan con OCR: pdf.js solo extrae texto embebido digitalmente. Para documentos escaneados, exporta las páginas como JPG o PNG y súbelas directamente: la herramienta aplicará OCR con Tesseract.js.',
  ...
  en: {
    ...
    faqA8: 'It depends on the file type. PDFs with digital text (exported from Word, generated by software) are converted directly. Scanned PDFs — where each page is an image — are not OCR-processed: pdf.js only extracts digitally embedded text. For scanned documents, export the pages as JPG or PNG and upload them directly: the tool will apply OCR using Tesseract.js.',
